...
- SQLite (included through the application dependency; no separate database server required)
...
```json
"ConnectionStrings": {
   "DefaultConnection": "Data Source=App_Data/contosodashboard.db"
}
```

The database file is created automatically in the `App_Data` directory when the application starts.
...
```powershell
Remove-Item .\App_Data\contosodashboard.db -ErrorAction SilentlyContinue
